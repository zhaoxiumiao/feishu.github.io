...
const addtest = async () => {
  ...
  const nameList2 = await table.getFieldMetaList();
  const nameF = await table.getFieldByName("BMID");
  const nameList = await nameF.getFieldValueList();
  const arr2 = nameList2.filter((field) => field.name.includes("绑定个人号"));
  const idList = arr2.map((f) => f.id);
  console.log("ID 数组 (可直接复制):", idList);
